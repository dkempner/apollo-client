...
// TODO: add documentation around `LocalState` here, not around `ApolloClient`.
...
> **Note**: The `addResolvers`, `setResolvers`, `getResolvers`, and `setLocalStateFragmentMatcher` methods have been removed from `ApolloClient`. Resolvers are now managed through the `LocalState` instance passed to the `localState` option.
...
## Deprecation notice
// TODO need to look at this more, too
